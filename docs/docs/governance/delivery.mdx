...
```
        ...
        └───────────────────────────┬────────────────────────────────┘
                    ...
                    └───────────────┬───────────────┘
                                    │  value + diff accepted, acceptance floor met
                    ┌───────────────▼───────────────┐
...
oracle. · *Gate:* the `merge-card` required check — a fresh non-author review approval on the head
sha, plus `value-fsm` green where there is a runtime surface to be green about.
**[AMENDED 2026-09-14 (`DEC-2026-09-14-7`): D9's `state: value-signed` label is retired as a gate
term.** It predated the uniform PR contract under which every PR already carries a non-author
reviewer PASS; for a docs/CI PR with no `pr-value` leg the label *was* the whole value bar, which
spent the scarcest human input on the changes least able to justify it. The non-author requirement
survives in the diff approval; multichannel corroboration of the attestation remains **TO BUILD**.]
...
maintainer-only on a passing template check; `claimed` only with a live lease; a PR merges only on
a green `merge-card` (non-author approval, `value-fsm` green where it applies) + security green;
`incoming` >3 days alarms; `needs-info` quiet 14
...
| `state: value-signed` | A non-author witnessed the value — the issue's lifecycle state. Since `DEC-2026-09-14-7` it is a record, not a gate: the merge bar is the `merge-card` check |
...
| D9 non-author validation, corroborated (label retired `DEC-2026-09-14-7`) | `merge-card` requires a fresh non-author approval + `value-fsm` green where applicable (merge-time) · **release-time:** `value-gate` requires every batch PR pr-value-green, or non-runtime + reviewed at merge · channel-consistency checking still **TO BUILD** | **have** (CI: `merge-card-gate` merge-time, `release-value-gate` release-time; [ADR-0029](https://github.com/Vexa-ai/vexa/blob/main/docs/adr/0029-release-witness-and-value-gates-enforced.md)/[ADR-0030](https://github.com/Vexa-ai/vexa/blob/main/docs/adr/0030-merge-card-value-and-diff-gate.md)) |
...
| Merge bar — value + diff accepted (choke point 1) | `merge-card` required status check: value-fsm green on head for a runtime PR (value) AND a fresh non-author review approval (diff); no label term since `DEC-2026-09-14-7` | **have** (CI: `merge-card-gate`, [ADR-0030](https://github.com/Vexa-ai/vexa/blob/main/docs/adr/0030-merge-card-value-and-diff-gate.md)) |
