...
| **Your agent space** | `POST /agent/chat`, and `/agent/*` → `chat`, `workspace/{tree,file,init,swap,git,upload}`, `sessions`, `routines`, `models` | owner = you, **by partition** (you can only name your own workspace/session) | none today |
| **Agent meeting views/controls** | `GET /agent/meeting/stream`, `POST /agent/meeting/process` | exact canonical meeting row is revalidated against the authenticated owner before any Redis read/write; native links are never carrier keys | none today |

Two safety shapes result:

- **Agent space** is safe by partition — a workspace, session, or routine is addressed by your id.
- **Meetings** are owned in the meetings domain, so the owner-checked path runs there (`GET /transcripts`,
  `/ws` subscribe authorization); the agent meeting stream/process paths consult that authority before
  touching their row-keyed carriers. Native-only `/agent/meeting/start` is retired (`410`).
...
| `can_access` / `OwnerOnlyPolicy` **adopted on every meeting path** | ⚠️ **partial** — meeting stream/process now bind an exact row to its owner through meeting-api; the common `can_access` policy is not yet the single implementation on every path |
| Server-side live-meeting **owner attribution** | ✅ **built** — meeting-api exposes only `{meeting_id,user_id}` on a secret-protected internal edge; the watcher validates the exact numeric row and canonical integer owner before live registration, consent reads, or dispatch, with no `u_live` fallback |
...
The front door and meeting-row mediation are real. The remaining cleanup is consolidation: today some
paths enforce the same owner-only rule directly through owner-scoped SQL/internal authority rather than
calling the common `OwnerOnlyPolicy` implementation.
...
|---|---|---|---|
| **P20 — complete mediation**: authorize every access, default-deny `canAccess` on API · WS · agent | meeting-api queries, WS subscribe, agent meeting context/stream/process, and watcher attribution all fail closed on exact owner authority; the shared `can_access` function is not yet their single implementation | consolidate the proven owner checks behind `can_access` without weakening the existing route-level denial tests | ADR-0012 · `gate:access` |
...
| **Multi-tenant attribution** | exact numeric meeting rows are resolved to their canonical database owner over the internal meeting-api edge; missing/unreachable/malformed authority creates no live row or worker | keep the owner edge covered by the deployment and cross-tenant gates | Auth spine · M2 |
