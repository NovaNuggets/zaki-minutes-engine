...
A live-meeting copilot is dispatched by the trusted transcript watcher from the meetings-domain numeric
row. The row id—not the reusable native meeting link—keys the transcript, processing cursor, retention
fence, and worker identity.

- `POST /api/meeting/process` `{ meeting_id, native_id, on }` → authenticated, owner-scoped processing
  consent. `meeting_id` must be the positive numeric row and `native_id` must match that owned row.
  `on:true` creates a new opaque worker generation and resumes from the per-meeting cursor
  (`{ ..., "processing":true, "resumed_from":"<id>" }`). `on:false` revokes that generation, stops the
  active runtime workload, and freezes the cursor (`{ ..., "processing":false }`). Authority or stop
  failures return `503`; foreign/unknown rows return indistinguishable `404`s.
- `POST /api/meeting/start` is retired and returns `410`: native-only dispatch was neither tenant-unique
  nor connected to the numeric retention fence.
...
- `GET /api/meeting/stream?meeting_id=&session_uid=` (SSE) → the merged transcript + copilot-output feed.
  Both parameters must be the same canonical positive meeting row owned by the authenticated user;
  native provider link ids are rejected as carrier keys.
...
`POST /api/workspace/upload` (multipart `files`, bounded per-file and in aggregate) →
`{ "files": [{ name, path }] }` ·
