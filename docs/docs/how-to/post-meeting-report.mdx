...
create the visibly named bot with `POST /bots`, opt the returned numeric meeting row into processing
with `POST /agent/meeting/process`, then stream cards from `GET /agent/meeting/stream`. See
