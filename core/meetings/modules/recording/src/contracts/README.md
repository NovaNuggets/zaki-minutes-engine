...
- [`recording-v1.md`](recording-v1.md) — the prose contract: chunk shape (`chunk_seq` + `is_final` +
  `format`), meeting-scoped `(meeting_id, session_uid)` identity on bot HTTP multipart, the desktop
  ingest-WS transport, and the master-assembly strategies (WebM byte-concat, WAV RIFF-merge).
