...
| `test_minutes_erasure_api.py` | L3 integration | exact signed Agent→Minutes erasure, pre-delete durable receipt, byte-stable retry/concurrency, tamper/race rejection, and exact OpenAPI response. |
| `test_minutes_platform_finalized.py` | L3 integration | separate operator artifact sink, secret-free Redis outbox, opt-in user delivery, and retry/restart recovery after finalizer failure. |
